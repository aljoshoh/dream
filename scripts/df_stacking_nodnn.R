...
for(i in 1:length(stack_features)){
  colnames(stack_features[[i]]) = c("mut.glm","mut.rf",#"mut.dnn",
                                    "rna.glm","rna.rf",#"rna.dnn",
                                    "clin.glm","clin.rf"#,"clin.dnn"
                                    )
}
dump_features(stack_features, path = "features/stacked_models_sc1_features_nodnn.RData")
auc_stack <- auc_rna[intsec,]
dump_features(auc_stack, path = "features/stacked_models_sc1_response_nodnn.RData")
#if(FALSE){
models_list_stacked <- run_pipeline_benchmark( # does not work because of the CV-built being out of bounds !
  feature_path = "features/stacked_models_sc1_features_nodnn.RData", # path to features, this time as list orderer like the drugs in the response path file !!!
  response_path = "features/stacked_models_sc1_response_nodnn.RData", # path to response
  submission = F,
  kfold = 10, 
  method = "rf",
  hyperparam = list(c(1,2,3,4,5,6),c(500)), #c("alpha"=0.5), #list(c(333),c(500)), # c("alpha"=0.5),
  cvglm = T,
...
dump_features(models_list_stacked_cv, path = "outputs/stacked_models_cv_sc1_nodnn.RData")
...
dump_features(models_list_stacked, path = "outputs/stacked_models_sc1_nodnn.RData")
