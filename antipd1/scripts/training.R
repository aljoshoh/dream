...
if(which == "ORR"){
  ...
  datac <- datac[!is.na(datac$Response),]
}
...
# Define learner
lrn_glm <- mlr_pipeops$get("learner", learner = mlr_learners$get("surv.cv_glmnet"))
lrn_names <- c("glm")
...
ps <- ParamSet$new(list(
  ParamInt$new("variance.filter.nfeat", lower = 5000, upper = 5000, default = 5000),
  ParamInt$new("performance.filter.nfeat", lower = 500, upper = 500, default = 500),
  #ParamDbl$new("mrmr.filter.nfeat", lower = 2, upper = 50),
  ParamDbl$new("surv.cv_glmnet.alpha", lower = 0.0, upper = 0.0),
...
ps$add_dep("performance.filter.nfeat",
...
fit <- survfit(Surv(time = OS, event = OSevent) ~ crank,
               data = data_train)
plt <- ggsurvplot(fit, conf.int=F, pval=TRUE, risk.table=TRUE)
pdf(paste0(getwd(),"/metadata/models/",model_id,"/","survivals_train.pdf"))
plt
dev.off()
#######################################
...
if(benchMark){
  ...
  cat(paste0(bmr$aggregate(measure),"\n"), file = logfile, append = T)
  cat("-------\n", file = logfile, append = T)
}
###############################################
