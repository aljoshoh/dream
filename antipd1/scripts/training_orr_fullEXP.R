...
which <- "ORR"
load("metadata/features_symbols_v2.RData")
load("metadata/response_v2.RData")
if(which == "OS"){
...
save(data, file  = "metadata/train_data_ORR_full.RData")
...
ps <- ParamSet$new(list(
  ParamInt$new("variance.filter.nfeat", lower = 1000, upper = 10000, default = 5000),
  ParamInt$new("importance.filter.nfeat", lower = 100, upper = 5000, default = 500),
...
# Save training
save(glrn, file = paste0(getwd(),"/models/",model_id,"/","untuned_ORR_full.RData"))
save(autotune, file = paste0(getwd(),"/models/",model_id,"/","tuned_full.RData"))
###############################################


# training set results
features_test <- get(load("metadata/features_symbols_v3_nanostring"))
